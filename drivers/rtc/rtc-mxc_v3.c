...
{
	printk("%s:wakeup_status : %d\n", __func__, wakeup_status);
	rtc_can_wakeup = wakeup_status;
	printk("%s:rtc_can_wakeup : %d\n", __func__, rtc_can_wakeup);
	return 0;
}
...
{
	struct rtc_drv_data *pdata = platform_get_drvdata(pdev);
	printk("%s:rtc_can_wakeup is %d, wakeup=%d\n", __func__, rtc_can_wakeup, device_may_wakeup(&pdev->dev));
