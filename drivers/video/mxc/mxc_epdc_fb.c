...
static inline void epdc_lut_complete_intr(u32 lut_num, bool enable)//屏蔽所有中断
{
	if (enable)
...
static inline void epdc_set_update_coord(u32 x, u32 y)
{
	...
	__raw_writel(val, EPDC_UPD_CORD);//插入坐標，高16位是y,低十六位是x
}
				       ...
				       u32 hsync_width, u32 hsync_line_length)
{
	u32 reg_val =
	    ...
	    | ((hsync_line_length << EPDC_TCE_HSCAN1_LINE_SYNC_OFFSET) &
	       EPDC_TCE_HSCAN1_LINE_SYNC_MASK);//高16位为有效的扫描长度，低16位为整行的长度
...
void epdc_init_settings(struct mxc_epdc_fb_data *fb_data)
{
	...
	epdc_set_temp(mxc_epdc_fb_get_temp_index(fb_data, g_eink_temp));//設置溫度參數到一個寄存器中，裏面函數是找到合適的溫度參數
	 ...
	 * VCOM_VAL = 0
	 * DDR_MODE = DISABLED
	 * LVDS_MODE_CE = DISABLED//LVDS低压差分信号
	 * LVDS_MODE = DISABLED
	 * DUAL_SCAN = DISABLED
	 * SDDO_WIDTH = 8bit
	 * PIXELS_PER_SDCLK = 4
...
//	printk("EPDC Powerup\n");
	...
	fp9928_i2c_device_read(REG_TMST_VALUE, &g_eink_temp);
	printk("<0>temp=%d\n", (int)g_eink_temp);
...
static int mxc_epdc_fb_get_temp_index(struct mxc_epdc_fb_data *fb_data, char temp)
{
	...
	/* if temperaturen < 0'c or temp > 100'c then use index : 0 */
	if (temp < 0 || temp > 100)
		return 0; 
			...
			if (!upd_data_list) {
				...
				upd_data_list =
					list_entry(fb_data->upd_buf_free_list.next,
						struct update_data_list, list);//list结构成员找到.next的指针（container_of）
...
/*	if(upd_data->waveform_mode == 3)
			upd_data->waveform_mode = 2;
*/
	...
	(upd_data->update_region.width >= 1020 && upd_data->update_region.height >= 750) || 
	(upd_data->update_region.width >= 750 && upd_data->update_region.height >= 1020)
	) {
		dev_dbg(fb_data->dev, "flush_updates");
		spin_unlock_irqrestore(&fb_data->queue_lock, flags);
		mxc_epdc_fb_flush_updates(fb_data);
		spin_lock_irqsave(&fb_data->queue_lock, flags);
		dev_dbg(fb_data->dev, "flush done");
	}
...
//	fb_data->cur_mode = &fb_data->pdata->epdc_mode[1]; //E60_V220
	...
	platform_set_drvdata(pdev, fb_data);//将info信息存放在参数driver_data中
	info = &fb_data->info;
