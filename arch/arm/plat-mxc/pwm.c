...
int pwm_config(struct pwm_device *pwm, int duty_ns, int period_ns)
{
	...
	if (!(cpu_is_mx1() || cpu_is_mx21())) {
		...
		period_reg = readl(pwm->mmio_base + MX3_PWMPR) + 2;
		if (period_reg != period_cycles) {
			printk("<0>Setting period reg old 0x%08x new 0x%08x\n", period_reg, period_cycles);
			writel(period_cycles - 2, pwm->mmio_base + MX3_PWMPR);
		}
		...
		if ((cr_reg & ~MX3_PWMCR_EN) != cr) {
			writel(cr | (cr_reg & MX3_PWMCR_EN), pwm->mmio_base + MX3_PWMCR);
			printk("<0>Setting cr old 0x%08x new 0x%08x\n", cr_reg, cr);
		}
